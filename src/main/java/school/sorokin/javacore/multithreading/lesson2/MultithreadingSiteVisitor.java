...
public class MultithreadingSiteVisitor {
     ...
     * Пояснение: быстро, так как ничего не блокируем, а просто выполняем операцию атомарно
     *
     * ReentrantLockCounter: 1.105 секунд. Значение счетчика: 10
     * Пояснение: медленно, так как блокируем монитор и другим потокам приходится ждать
     *
     * SynchronizedBlockCounter: 1.075 секунд. Значение счетчика: 10
     * Пояснение: медленно, так как блокируем монитор и другим потокам приходится ждать
     *
     * UnsynchronizedCounter: 0.104 секунд. Значение счетчика: 8
     * Пояснение: значение не соответствует ожидаемому, так произошел race condition
     *
     * VolatileCounter: 0.112 секунд. Значение счетчика: 10
     * Пояснение: здесь тоже есть вероятность race condition
     */
